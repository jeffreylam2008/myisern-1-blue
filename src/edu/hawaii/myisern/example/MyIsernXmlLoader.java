...
import com.meterware.httpunit.WebConversation; //import com.meterware.httpunit.WebLink;
...
public class MyIsernXmlLoader {
    ...
    for (Researcher researcher : unmarshalledResearchers.getResearcher()) {
      ...
      if (isValidResearcher(researcher)) {
        ...
        if (this.uniqueIdList.add(uniqueId)) {
          ...
          // if not successful, log name exists & move on to next
          System.out.println("Name: " + researcher.getName() + " already exists in the database.");
        }
  ...
  /**
   * Overloads isValidYear to handles a list of years. Method finds most recent year and validates
   * it.
   * 
  ...
  private static boolean isValidYear(List<BigInteger> yearList) throws Exception {
    try {
      ...
      for (BigInteger year : yearList) {
        /*
         * b = bi1.compareTo(bi2); Returns negative number if bi1<bi2, 0 if bi1==bi2, or positive
         * number if bi1>bi2.
         */
  ...
  private boolean isValidResearcher(Researcher researcher) throws Exception {
    try {
      ...
      return (isValidString(researcher.getName()) && isValidString(researcher.getOrganization())
  ...
  private boolean isValidOrganization(Organization org) throws Exception {
    try {
      ...
      // valid home-page
      // valid web link
      return (isValidString(org.getName()) && isValidString(org.getType())
          && isValidString(org.getContact()) && isValidString(affilateResearcherList)
          && isValidString(org.getCountry()) && isValidString(researchKeywordList)
          && !researchKeywordList.isEmpty() && isValidString(org.getResearchDescription())
  ...
  public static boolean isValidCollaboration(Collaboration collab) throws Exception {
    try {
      ...
      // valid Description
      // valid string
      return (isValidString(collab.getName()) && isValidString(collabOrgList)
          && (collabOrgList.size() > 1) && isValidString(collabTypeList)
          && !collabTypeList.isEmpty() && isValidYear(collab.getYears().getYear())
          && isValidString(outcomeTypeList) && !outcomeTypeList.isEmpty() && isValidString(collab
          .getDescription()));
    }
    catch (Exception e) {
      return false;
